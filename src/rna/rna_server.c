  ...
  *type = META_ALIGNMENT_MIDDLE;
  ...
  if (l_found && r_found) {
	...
	if (size_ex_l > 1) {
	  ...
	  if ((s_prev_read_end + size_ex_l + 1) >= strlen(query_map)) {
	    printf("ERROR detected in file %s and line %i! Continue mapping...\n", __FILE__, __LINE__);
	    return NULL;	    
	  }
