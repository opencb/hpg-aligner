...
char *cigar_code_find_and_report_sj(size_t start_map, cigar_code_t *cigar_code, 
  ...
  if (num_sj > 0) {
	...
	/*
	size_t lim_inf = 43380613;
	size_t lim_sup = 43583573;

	if ((lim_inf - 2 <= sj_start && sj_start <= lim_inf + 2) &&
	    (lim_sup - 2 <= sj_end && sj_end <= lim_sup + 2)) {
	  printf("%s : %s, %lu, %i(%s)\n", read->id, cigar_str, start_map, chromosome, new_cigar_code_string(cigar_code));
	}
	*/
  ...
  return report_cig == 1 ? strdup(cigar_str) : NULL;
...
void fastq_read_revcomp(fastq_read_t *read) {
  ...
  for (i = read->length - 1, j = 0; i >= 0; i--, j++) {
    read->revcomp[j] = convert_ASCII[(unsigned char)read->sequence[i]];
  }
...
cigar_code_t *generate_cigar_sw_output(char *seq_sw, 
  ...
  *node_avl_end   = NULL;
  ...
  while (j < map_sw_len) {
	...
	if ((found != NOT_SPLICE) || 
	    (end_splice - start_splice + 1 < min_intron) ||
	    (end_splice - start_splice + 1 > max_intron)) {
	    array_list_clear(cigar_code->ops, (void *)cigar_op_free);
	    cigar_code_free(cigar_code); 
	    return NULL; 
	}
...
cigar_code_t *fill_extrem_gap(char *query, 
  ...
  if (type == FILL_GAP_LEFT) {
    //printf("FILL_GAP_LEFT\n");
    seed_region_t *s_prev = linked_list_get_first(cal->sr_list);
    if (s_prev == NULL) { return cigar_code; }
...
void meta_alignment_close(meta_alignment_t *meta_alignment) {
  ...
  //printf("%i, cigar_code ops = %i\n", bad_cigar, array_list_size(cigar_code->ops));
...
void merge_seeds_cal(cal_t *cal) {
  ...
  linked_list_item_t *list_item = cal->sr_list->first;
  //if (list_item == NULL) { 
...
void sw_depth_process(sw_optarg_t *sw_optarg, sw_multi_output_t *output, 
  ...
  if (sw_depth->depth == MAX_DEPTH || 
	...
	//printf("3.SW CIGAR %s, %i\n", new_cigar_code_string(cigar_code), cigar_code->distance);
...
*/
  ...
  *distance = mismatches[max_sp_pos];
  ...
  if (*distance > max_distance || 
      (int)genome_start_aux - (int)genome_end_aux - 1 < min_intron) {
    intron_size = 0;
    goto exit;
  }
...
int generate_cals_between_anchors (int mode,
  ...
  seed_region_t *s_prev = NULL, *s_next;
  ...
  if (mode == CALING_DOUBLE_ANCHORS ) {
    if (last_cal->start - first_cal->end < max_intron) {
      min_cal_size = 15;      
    }
  }
  ...
  if (found) {
    for (int j = num_cals - 1; j >= 0; j--) {
      if (founds[j]) {
	//printf("Free CAL, %i items\n", array_list_size(cal->sr_list));
...
int apply_sw_rna(sw_server_input_t* input_p, batch_t *batch) {
  ...
  //max_intron_size = input_p->max_intron_size;
  ...
  for (i = 0; i < num_reads; i++) {        
	...
	//cal_print(last_cal);
	if (first_cal->end + min_intron/*fq_read->length*/ >= last_cal->start) {
	  found[p] = 1;
	  found[p + 1] = 1;
	  founds++;
	} else {
  ...
  for (int i = 0; i < num_reads; i++) {
    ...
    int map = 0;
    meta_alignment_t *meta_alignment = array_list_get(0, meta_alignments_list[i]);
    int delete_meta[array_list_size(meta_alignments_list[i])];
    int start_delete = 0;
	...
	//printf("SCORE: %i\n", );
        ...
        int read_score = get_score(cigar_code, fq_read, match, mismatch, gap_open, gap_extend);      
	...
	alignment->alig_data = cigar_code_dup(cigar_code);
    ...
    for (size_t a = 0; a < n_alignments; a++) {
      ...
      alignment->optional_fields = (uint8_t *)optional_fields;
    }
...
int apply_rna_last(sw_server_input_t* input_p, batch_t *batch) {
  ...
  array_list_t *cals_list, *fusion_cals;
  cal_t *cal, *first_cal, *last_cal;
  fastq_read_t *fq_read;
  ...
  for (i = 0; i < num_reads; i++) {
    ...
    //printf("WK_2ph-Process: == (%i CALs, %i)Read %s ==\n", array_list_size(cals_list), array_list_get_flag(cals_list), fq_read->id);
    ...
    */
    ...
    int read_nt;
    cigar_code_t *cigar_code_res;
    int n_cals = array_list_size(cals_list);
    int free_cals[n_cals];

    for (int p = 0; p < n_cals; p++) {
      cal = array_list_get(p, cals_list);
      free_cals[p] = 0;
	...
	array_list_set_flag(array_list_get_flag(mapping_batch->mapping_lists[i]), cal_list_aux);
	  ...
	  */
	  ...
	  if (array_list_size(fusion_list) > 1) { 
	    ...
	    if (cigar_code != NULL) {
	      ...
	      array_list_t *fusion_list_aux = array_list_new(5, 1.25f, COLLECTION_MODE_ASYNCHRONIZED);
	      meta_alignment = meta_alignment_new();
	      meta_alignment_insert_cal(first_cal, meta_alignment);
	      array_list_insert(first_cal, fusion_list_aux);
	      int target_del = array_list_size(fusion_list);
		...
		meta_alignment_insert_cal(last_cal, meta_alignment);
		meta_alignment_insert_cigar(cigar_code, CIGAR_SIMPLE_MIDDLE, 0, meta_alignment);
		array_list_insert(last_cal, fusion_list_aux);
		target_del--;
	      }
	      
	      for (int z = 1; z < target_del; z++) {
		cal_t *cal_tmp = array_list_get(z, fusion_list);
		cal_free(cal_tmp);
	      }
	      
	      array_list_free(fusion_list, (void *)NULL);
	      fusion_list = fusion_list_aux;
      ...
      //Meta-alignment found in first phase
      cals_list = mapping_batch->mapping_lists[i];
      n_cals = array_list_size(cals_list);
  ...
  for (int i = 0; i < num_reads; i++) {
	...
	array_list_get_flag(mapping_batch->mapping_lists[i]) != BITEM_SINGLE_ANCHORS) { 
      //printf("Continue %i, %i\n", array_list_size(meta_alignments_list[i]), array_list_get_flag(mapping_batch->mapping_lists[i]));
      continue;
    }
  ...
  for (int i = 0; i < num_reads; i++) {
    ...
    //int delete = 0;
    //int delete_array[array_list_size(meta_alignments_list[i])];
	...
	if (meta_alignment_get_status(meta_alignment) == META_CLOSE) { 
	  ...
	  if (h_left > fq_read->length || h_left < 0) { 
	    meta_alignment_complete_free(meta_alignment);
	    continue; 
	  }

	  if (h_left + h_right >= fq_read->length) { 
	    meta_alignment_complete_free(meta_alignment);
	    continue; 
	  }
