...
stats_options_t *stats_options_new(char *exec_name, char *command_name) {
  ...
  opts->verbose = 0;
  opts->help = 0;
  opts->num_threads = get_optimal_cpu_num_threads();
  opts->batch_size = 10000;

  opts->quality_encoding_value = 0;

  // filter or trim stats_options
...
stats_options_t *stats_options_parse(char *exec_name, char *command_name,
  ...
  if (argc < 2) {
    ...
    if (num_errors > 0) {
      fprintf(stdout, "\nError:\n");
      arg_print_errors(stdout, argtable[NUM_STATS_OPTIONS], "\t");
      usage_argtable(exec_name, command_name, argtable);
    } else {
...
void stats_options_display(stats_options_t *opts) {
  ...
  printf("\tK-mers (5-mer)       : %s\n", (opts->kmers_on ? "Enabled" : "Disabled"));
...
stats_options_t *read_cli_stats_options(void **argtable, stats_options_t *opts) {	
  ...
  if (((struct arg_str*)argtable[5])->count) { opts->quality_encoding_name = strdup(*(((struct arg_str*)argtable[5])->sval)); }
  if (((struct arg_int*)argtable[6])->count) { opts->kmers_on = (((struct arg_int*)argtable[6])->count); }
  if (((struct arg_str*)argtable[7])->count) { opts->read_length_range = strdup(*(((struct arg_str*)argtable[7])->sval)); }
  if (((struct arg_str*)argtable[8])->count) { opts->read_quality_range = strdup(*(((struct arg_str*)argtable[8])->sval)); }
  if (((struct arg_int*)argtable[9])->count) { opts->left_length = *(((struct arg_int*)argtable[9])->ival); }
  if (((struct arg_str*)argtable[10])->count) { opts->left_quality_range = strdup(*(((struct arg_str*)argtable[10])->sval)); }
  if (((struct arg_int*)argtable[11])->count) { opts->right_length = *(((struct arg_int*)argtable[11])->ival); }
  if (((struct arg_str*)argtable[12])->count) { opts->right_quality_range = strdup(*(((struct arg_str*)argtable[12])->sval)); }
  if (((struct arg_int*)argtable[13])->count) { opts->max_N = *(((struct arg_int*)argtable[13])->ival); }
  if (((struct arg_int*)argtable[14])->count) { opts->max_out_of_quality = *(((struct arg_int*)argtable[14])->ival); }
...
void** new_argtable_stats_options() {
  ...
  argtable[1] = arg_file1("f", "fastq-file", NULL, "Input file name (FastQ format)");
  ...
  argtable[5] = arg_str0(NULL, "quality-encoding", NULL, "Encoding for quality scores: phred33, phred64");
  argtable[6] = arg_lit0(NULL, "kmers", "Enable k-mers analysis (5-mer)");
  argtable[7] = arg_str0(NULL, "read-length-range", NULL, "Read length range, eg. 80,110");
  argtable[8] = arg_str0(NULL, "read-quality-range", NULL, "Read quality range, eg. 20,40");
  argtable[9] = arg_int0(NULL, "left-length", NULL, "Number of leftmost nucleotides to take into account to filter or trim");
  argtable[10] = arg_str0(NULL, "left-quality-range", NULL, "Quality range for the leftmost nucleotides, eg. 15,45");
  argtable[11] = arg_int0(NULL, "right-length", NULL, "Number of rightmost nucleotides to take into account to filter or trim");
  argtable[12] = arg_str0(NULL, "right-quality-range", NULL, "Quality range for the rightmost nucleotides, eg. 10,60");
  argtable[13] = arg_int0(NULL, "max-N", NULL, "Maximum number of Ns in the sequences");
  argtable[14] = arg_int0(NULL, "max-out-of-quality", NULL, "Maximum number of nucleotides out of the read quality range");
