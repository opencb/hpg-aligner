  ...
  for (i=0, j=0; (i<(ll1-2)) && (j<ll2); i++, j++){
    if (st1[i]==st2[j]){
	  ...
	  *****/
      ...
      return (-1.0f);
	  }//---> para no concatenar mas de TOPE_ERRORES consecutivos. (27-XI-2013)
	    else {
	      errores++;
	      if (errores == 1) {
		first_i = i;
	  ...
	  //----------------------->> sigo: hay pocos errores consecutivos.  
...
if(i==(ll1-2)){ //--> solo si estoy en los dos ultimos!!
...
//  printf("\t\t\tscore = %0.2f (mapped %i of %i): matches = %d, mismatches = %d, open-gaps = %d, extended-gaps = %d\n", 
//	 score, *map_len1, ll1, *match, *mism, *gap1, *gapmas);
  printf("\t\t\tscore = %0.2f (query: mapped %i of %i, ref: mapped %i of %i): matches = %d, mismatches = %d, open-gaps = %d, extended-gaps = %d\n", 
	 score, map_len1, ll1, map_len2, ll2, match, mism, gap1, gapmas);
  #endif
...
// printf("\n\n i=%d  j=%d match=%d mism=%d  gap1=%d  gapmas=%d  s_m_l=%d ",
//         map_len1, map_len2, match, mism, gap1, gapmas, st_m_len);
//getchar();
...
/****
	out->map_len1 = map_len1;
    out->map_len2 = map_len2;
    out->match = match;
    out->mismatch = mism;
    out->gap_open = gap1;
    out->gap_extend = gapmas;
	out->st_map_len = st_m_len;
****/
  
// printf("\n\n i=%d  j=%d match=%d mism=%d  gap1=%d  gapmas=%d  s_m_l=%d ",
//         out->map_len1, out->map_len2, out->match, out->mismatch, 
//		 out->gap_open, out->gap_extend, out->st_map_len); 
...
//  *match=0; *mism=0; *gap1=0; *gapmas=0; 
  ...
  */
  ...
  if(i==1){ //--> solo si estoy en los dos ultimos!!
    if (st1[i]==st2[j]){ //--> coincide 1
#ifdef _VERBOSE
      set_map_trace(&map_counter, st1_map, st1[i], nt_map, '|', st2_map, st2[j]);
#endif
